function heldKarpUndirected() {
    ...
    heldKarpHelpUndirected(wm, root);
}
 ...
 */
function heldKarpHelpUndirected(wm, x) {
    let n = wm.length;
    var memo = new Map();
    ...
    for (let i = 0; i < nums.length; i++) {
        ...
        tempNums.splice(i, 1);
        let h = heldKarpPathUndirected(nums[i], x, tempNums, wm, memo);
 ...
 */
function heldKarpPathUndirected(x, rt, nums, wm, memo) {
    let nodes = getAllNodes(cy);
    cy.elements(':grabbable').unselect();
    if (nums.length === 0) {
        ...
        for (let i = 0; i < nums.length; i++) {
            ...
            if (memo.get(keyStr) !== undefined) {
                ...
                h = heldKarpPathUndirected(nums[i], rt, tempNums, wm, memo);
                memo.set(keyStr, h);
            }
...
/**
 * 
 * @param {boolean} dir directed or not
 */
